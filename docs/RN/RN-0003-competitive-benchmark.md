...
- 2026-07-28: **`get` di-tweak** — gabung lookup entity (satu akses `entities[i]`: cek hidup+generasi+lokasi) menghapus indeks redundan. get ~20–24 (noisy). **Sisa gap `get` bersifat arsitektural** — rantai indireksi entity→meta→lokasi→archetype + **downcast type-erasure** per akses; menutupnya penuh butuh `unsafe` downcast-tak-tercek (melanggar disiplin unsafe-terkurung untuk gain marginal) atau layout storage lebih datar (perubahan desain). **Dihentikan di ~1.5× hecs** — trade-off jujur: arke pilih 0-`unsafe`-pengguna + type-erasure aman di atas beberapa ns pada akses acak. iter2/spawn (jalur lebih panas) sudah kompetitif/menang.
