...
| **v3** (M-20) | Materialisasi **query-scoped**: `load_where::<T>(world, predicate)` memuat subset entity yang cocok predikat SQL atas kolom `T` beserta seluruh komponennya (working-set parsial; aman dengan `save_incremental`). **Index/constraint kustom**: atribut derive `#[pg(index)]`/`#[pg(unique)]` (per-field) & `#[pg(check = "…")]` (per-tipe) → `migrate` membuat indeks & constraint `CHECK` (idempoten). |
