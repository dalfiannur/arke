...
| Crate | Versi | Isi |
| --- | --- | --- |
| [`arke-postgres`](arke-postgres/) | [![crates.io](https://img.shields.io/crates/v/arke-postgres.svg)](https://crates.io/crates/arke-postgres) [![docs.rs](https://img.shields.io/docsrs/arke-postgres)](https://docs.rs/arke-postgres) | Persistensi PostgreSQL — Postgres sebagai **sumber kebenaran** relasional berkolom-tipe. Tulis: `save` / `save_incremental` (diff) / `update_entity` (optimistic-lock). Baca: `load` / `load_where::<T>` (query-scoped) / **query builder typed** `query::<T>().filter(T::field().lt(x)).order_by(..).limit(..).load()` (RFC-0030) / **relasi entity + join** `query::<T>().join(T::rel(), R::f().lt(x))` ([RFC-0031](docs/RFC/RFC-0031-persistent-entity-relations-join.md)) — relasi bertipe `Ref<T>` + path bersarang 3–4 deep (`matches` / `.through().load_all()` / `descendants_of`/`ancestors_of`, [RFC-0032](docs/RFC/RFC-0032-nested-relations-recursive.md)). Anti-injeksi. Skema: `migrate` (reconciling) + `#[pg(index/unique/check)]`. Tipe: skalar / `Option` / `JSONB` / `NUMERIC`. Lihat [RFC-0021](docs/RFC/RFC-0021-arke-postgres-adapter.md). |
| [`arke-cache`](arke-cache/) | [![crates.io](https://img.shields.io/crates/v/arke-cache.svg)](https://crates.io/crates/arke-cache) [![docs.rs](https://img.shields.io/docsrs/arke-cache)](https://docs.rs/arke-cache) | Cache **read-through** opsional di depan `arke-postgres` — backend **Redis/DragonflyDB/KeyDB**. Postgres tetap sumber kebenaran; invalidate-on-write + TTL. Untuk baca point-read berat pada Postgres remote. Lihat [RFC-0033](docs/RFC/RFC-0033-cache-adapter.md). |
